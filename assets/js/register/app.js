...
jQuery(document).ready(function ($) {
    ...
    layout.on('scroll', function (e) {
        if ((layout.scrollTop() === layout.children('ul').outerHeight(true) - layout.height()) || APP.getOffSet() < 1) {
            if(!APP.is_request && APP.is_not_list() && APP.getOffSet() < getLimit() && APP.isOnline()){
                APP.is_request = true;
                var category = null;
                ...
                var product_ids = [];
                $.when(APP.getServerProducts(APP.getRequestLimit(), APP.getOffSet(), $('.cat_title:not(#wc-pos-register-grids-title)').first().data('parent'), APP.getIncludes())).then(function(products, status, jqXHR){

                    if(products.length > 0){
                        APP.db.putAll('products', products);
    ...
    APP = window.POS_APP = {
        ...
        getServerProducts: function (limit, offset, category, includes) {
            var e = $.get(wc_pos_params.wc_api_url + 'products/',
                {
                    per_page: limit,
        ...
        getServerOrders: function (reg_id, search) {
        ...
        setCustomer: function (customer, open) {
            var customer_id = is_object(customer) ? customer.id : customer;
            CUSTOMER.reset();
            if (!empty(customer_id) && parseInt(customer_id) > 0) {
                ...
                if (pos_grid.grid_id == 'categories') {
                    ...
                    $.each(cats, function (i, cat) {
                        ...
                        if(cat.parent == 0){
                            ...
                            $.when(APP.getServerProducts(50, 0, cat.term_id)).then(function(products, status, jqXHR){
                                APP.db.putAll('products', products);
                                $.each(products, function( i, product){
                                    if(wc_pos_params.image_size == 'thumbnail'){
                                        if(product.thumbnail_src){
                                    ...
                                    //store.products.push(product);
                                    pos_grid.products_sort.push(product.id);
                                    APP.lastOffset[cat.term_id]++;
                                    APP.limits[cat.term_id] = jqXHR.getResponseHeader("x-wp-total");
                                });
                            }).always(function(){
                                $li.css({
        ...
        createOrder: function (paid, paymentSense) {
            ...
            $.each(cart_contents, function (index, item) {
                ...
                if (_item.tax_status == 'none') {
                    ...
                    _item.subtotal_tax = item.line_subtotal_tax.toString();
                    _item.tax_data = item.line_tax_data;
                    _item.total_tax = item.line_tax.toString();
                }
        ...
        create_refund: function(){
            ...
            CART.pos_refund["order_refund"]["api_refund"] = false;
            ...
            if (wc_version >= 3) {
                ...
                $.fn.select2.amd.define("ProductsResultsAdapter", [
                    ...
                    ProductResults.prototype.showLoading = function (params) {
                        ...
                        var loadingMore = "Searching for " + params.term;
                ...
                $.fn.select2.amd.require([
                        ...
                        if(result.length >  0){
                            ...
                            request = $.get({
                                ...
                                success: function(response){
                                    ...
                                    //store.products = store.products.concat(response);
                    ...
                    $("#add_product_id").select2({
                        ...
                        resultsAdapter: $.fn.select2.amd.require("ProductsResultsAdapter"),
                    }).change(function () {
                        var val = $(this).val();
                        $(this).html('');
                        if (val != '') {
        ...
        setUpCustomerModal: function(_openModal, setAccountFunds, event){
            ...
            modal_order_customer.on("change", "#billing_country, #shipping_country", function () {
                ...
                $("#"+type+"_city_field").after($(CUSTOMER[type + '_address']['state_field']));
        ...
        is_not_list: function(product){
            if(pos_grid.grid_id == "all" || pos_grid.grid_id != "categories"){
                ...
                var category = $('.cat_title:not(#wc-pos-register-grids-title)').first().data('parent');
                if(typeof product !== "undefined"){
                    return $(".hndle").find("span.attr_title").length < 1 && in_array(category, product.categories_ids);
                }else {
        ...
        getOffSet: function(type){
            if(pos_grid.grid_id == "all"){
                ...
                var category = $('.cat_title:not(#wc-pos-register-grids-title)').first().data('parent');
                return APP.lastOffset[category];
