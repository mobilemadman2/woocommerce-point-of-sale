...
jQuery(document).ready(function ($) {
    ...
    function user_search(term) {
        ...
        $.ajax({
            ...
            data: {
                action: 'wc_pos_get_user_by_term',
                term: term,
                async: false
            },
            success: function (responce) {
                window.customers = responce;
            }
        });
...
// ajaxSetup is global, but we use it to ensure JSON is valid once returned.temp@_@admintemp
    ...
    APP = window.POS_APP = {
        ...
        sync_ProductData: function (block) {
            ...
            //     }
            // });
            // APP.insertUpdate(100, APP.lastOffset);
        ...
        getServerProductsCount: function () {
            var e = $.getJSON(wc_pos_params.wc_api_url + 'products/count/');
            return e;
        },
        getServerProducts: function (limit, offset) {
            var e = $.getJSON(wc_pos_params.wc_api_url + 'products/', {
        ...
        getServerOrdersCount: function (reg_id, search) {
            ...
            var e = $.post(wc_pos_params.ajax_url, {
                action: "wc_pos_get_served_orders_count",
                reg_id: reg_id,
                filter: filter,
        ...
        getServerOrders: function (opt) {
            ...
            var e = $.post(wc_pos_params.ajax_url , {
                action: "wc_pos_get_served_orders",
                reg_id: opt.reg_id,
                status: wc_pos_params.load_order_status,
                filter: filter
            });
        ...
        couponsUpdate: function (limit, offset, CouponsCount) {
            ...
            $.when(APP.getServerCoupons(limit, offset)).then(function (couponsData) {
                if (couponsData.coupons.length > 0) {
                    APP.coupons = couponsData.coupons;
        ...
        getServerCouponsCount: function () {
            var e = $.post(wc_pos_params.ajax_url, {
                action: "wc_pos_get_served_coupons_count",
                async:false
        ...
        getServerCoupons: function (limit, offset) {
            ...
            var e = $.post(wc_pos_params.ajax_url, {
                action: "wc_pos_get_served_coupons",
                async:false
        ...
        getServerCustomersCount: function () {
            var e = $.post(wc_pos_params.ajax_url, {
                action: "wc_pos_get_customers_count",
                role: "all",
                async: false
        ...
        loadOrder: function (order_id) {
            // APP.db.get('orders', order_id).always(function (order) {
                ...
                if (typeof order != 'undefined') {
                    ...
                    APP.showNotice(pos_i18n[4], 'error');console.log('2');
                }
            // });
        },
        addToCart: function (product_id, quantity, variation_id, variation, cart_item_data, item_id, item, loaded) {
            ...
            window.POS_APP.db.get = function (what, id)  {
                return APP.db[id];
            };
            record = APP.db.get('products', product_id);
        ...
        setCustomer: function (customer_id, open) {
            ...
            if (customer_id != '' && parseInt(customer_id) > 0) {
                ...
                $.ajax({
                    url: wc_pos_params.wc_api_url + 'customers/' + customer_id, success: function (response) {
                        if (response.customer.user_meta && typeof response.customer.user_meta.wp_user_avatar != 'undefined' && response.customer.user_meta.wp_user_avatar[0]) {
                            $.ajax({
                                type: 'POST',
                                url: wc_pos_params.ajax_url,
                                data: {
        ...
        updateSearchList: function (x) {
            $.each(POS_APP.db, function (i, val) {
                if(typeof val === "object"){
                    APP.insertSearchListItem(val);
                }
            });
        },
        insertSearchListItem: function (obj) {
            if(APP.tmp.products[obj.id]){
                return;
            }
        ...
        checkStock: function (product_data, quantity, cart_item_key) {
            try {
                ...
                if (CART.has_enough_stock(product_data, quantity) === false) {
                    throw new Error(sprintf(pos_i18n[4], product_data.title, product_data.stock_quantity)); console.log('1');
                }
        ...
        search_customer_wc_3: function (query, callback) {
            ...
            var result = [];
            var chk = {};
            $.when(user_search(query.term)).done( function (users) {
                $.each(window.customers, function (index, val) {
                    var fullname = [val.first_name, val.last_name];
                    fullname = fullname.join(' ').trim();
                    if (fullname == '') {
                        fullname = val.username;
            ...
            //             if (typeof callback == 'undefined') {
            //                 APP.add_customer_item_to_result({
            //                     id: val.id, avatar_url: val.avatar_url, fullname: fullname
            //                 });
            //             }
            //         }
            //     });
        ...
        ready: function () {
            ...
            if (wc_version >= 3) {
                $.fn.select2.amd.require([
                    ...
                    ProductData.prototype.query = function (params, callback) {
                        ...
                        var results = setUpSearchData(_term);
                        if(!results.length || results.length < 1){
                            ...
                            s_request = $.ajax({
                                ...
                                type: 'POST',
                                data: {
                                    action: "wc_pos_get_products_by_search",
                                    term: _term
                                },
                                success: function(response){
                                    if(typeof response.data === "object" && Object.values(response.data).length > 0){
                                        $.each(response.data, function (i, product) {
                                            POS_APP.db[product.id] = product;
                                            APP.db[product.id] = product;
                                        });
                                        APP.updateSearchList();
                                        results = setUpSearchData(_term);
                                    }
            ...
            if ($('#customer_user').length) {
                $("#customer_user").select2({
                    ...
                    ajax: {
                        ...
                        processResults: function (data) {
                            ...
                            $.each(data, function(i, user){
                                ...
                                fullName += ' (' + user.email + ' / ' + ' ' + user.phone + ')';
                                results.push({
                                    id: user.id,
                                    text: fullName,
                                    userData: user
                                ...
                                fullName = fullName.replace(/(cows)/g, '<span class="smallcaps">$1</span>');
                                APP.add_customer_item_to_result({id: user.id, avatar_url: user.avatar_url, fullname: fullName});
                            });
                    ...
                    //@todo: this template compiling occurs too many times. have to fix it.
                    var source = $('#tmpl-cart-customer-item').html();
                    var template = Handlebars.compile(source);
                    var html = template(e.params.data.userData);
                ...
                //
                //     $("#customer_user").select2({
                //         minimumInputLength: 3, multiple: true, dataAdapter: CustomerData
                //     }).change(function () {
                //         var customer_id = $(this).val();
                //         $("#customer_user").html('');
                //         APP.setCustomer(customer_id, wc_pos_params.load_customer);
    ...
    function setUpSearchData(_term){
        var result = [];
        var title = "";
        $.each(APP.tmp.products, function (index, o) {
            if ( o.post_meta && typeof o.post_meta[wc_pos_params.scan_field] != 'undefined' && wc_pos_params.ready_to_scan !== 'no') {
