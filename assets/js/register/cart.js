jQuery(document).ready(function ($) {
    ...
    window.POS_CART = {
            ...
            try {
                ...
                if (product_data.sold_individually === true) {
                    ...
                    if (in_cart_quantity > 0) {
                        throw new Error(sprintf(pos_i18n[1], product_data.name));
                    }
                }
                ...
                if (variation_data) {
                    ...
                    if (CART.has_enough_stock(variation_data, quantity) === false) {
                        if (wc_pos_params.decimal_quantity == 'yes' && !cart_item_key && variation_data.stock_quantity > 0) {
                            ...
                            throw new Error(sprintf(pos_i18n[4], product_data.name, variation_data.stock_quantity));
                        }
                    }
                    ...
                    if (product_data.in_stock === false && product_data.backorders_allowed === false) {
                        throw new Error(sprintf(pos_i18n[3], product_data.name));
                    }
                ...
                if (cart_item_key) {
                    ...
                    new_quantity = Math.round(new_quantity * 100) / 100;
        ...
        has_enough_stock: function (product_data, quantity) {
            ...
            ;
            var has_enough_stock = product_data.manage_stock === false || product_data.backorders_allowed === true || product_data.stock_quantity >= quantity ? true : false;
        ...
        calculate_totals: function () {
            ...
            $.each(cart, function (cart_item_key, values) {
                ...
                if(_product.tax_class === "parent"){
                    _product.tax_class = values['data']['tax_class'];
                }
                 ...
                 * No tax to calculate
                 */
                if (_product.tax_status !== "taxable" || wc_pos_params.pos_calc_taxes === false) {
            ...
            if (CART.fee_total > 0) {
                ...
                $.each(CART.fees, function (index, fee) {
                    var amount = fee.total;
                    if (fee.tax_status == 'taxable') {
                        var taxes = TAX.calc_exclusive_tax(fee.total, tax_rates);
        ...
        calculate_fees: function () {
            ...
            $.each(CART.fees, function (k, fee) {
                ...
                if (fee.type === 'percent') {
                    ...
                    $.each(CART.cart_session_data.fees, function(i, s_fee){
                        if(fee.name == s_fee.name){
                            CART.cart_session_data.fees[i]['total'] = amount;
                            return false;
                        }
                    });
                    CART.fees[k]['total'] = amount;
        ...
        add_custom_fee: function (fee) {
            CART.cart_session_data.fees.push(fee);
            CART.calculate_totals();
            if (wc_pos_params.tabs_management) {
        ...
        remove_fee: function (fee) {
            var index = CART.cart_session_data.fees.findIndex(function (fees) {
                return fees.name == fee;
            });
            if(index > -1){
                CART.cart_session_data.fees.splice(index, 1);
                return true;
            }
            return false;
        },
