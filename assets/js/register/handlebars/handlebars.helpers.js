Handlebars.registerHelper('displayProductItemTitle', function() {
    ...
    if( wc_pos_params.user_can_edit_product == true && product.id != pos_custom_product.id ){
    	...
    	title = '<span class="product_title" >'+title+'</a>';
    }
...
Handlebars.registerHelper('displayOrderTitle', function() {
    ...
    if( wc_pos_params.user_can_edit_order == true ){
	    ...
	    if(customer > 0 ){
	        ...
	        if(this.billing.first_name != '' || this.billing.last_name != ''){
	        	title += this.billing.first_name + ' ' + this.billing.last_name;
	        }else if( typeof customer.username != 'undefined' && customer.username != ''){
	        	title += customer.username;	        	
	        }
	        title += '</a>';
	    }else if( this.billing.first_name != '' || this.billing.last_name != ''){
	        title += this.billing.first_name + ' ' + this.billing.last_name;
	    }else{
    ...
    if( this.billing.email != '' ){
        title += '<small class="meta email">';
            title += '<a href="mailto:'+this.billing.email+'">';
                title += this.billing.email;
            title += '</a>';
        title += '</small>';
    }
	...
	if(wc_pos_params.image_size == 'thumbnail'){
		if(product.thumbnail_src){
			...
			if(product.images.length > 0){
				var string = '<img width="90" height="90" src="'+product.images[0].src+'" class="attachment-shop_thumbnail wp-post-image">';
			}
		}
	}
