(function ($) {
        ...
        if (options == 'destroy') {
            return this.each(function () {
                if ($(this).data('category_cycle') == 'init') {
                    ...
                    $_this.data('elems', '');
                    $_this.find('ul').off('click', '.open_category');
                    $_this.off("click");
    ...
    function get_filter(elems, parent, title, offset, limit, scrolled) {
        ...
        $(".title_product").remove();
        if (pos_grid.grid_id != 'all' && pos_grid.grid_id != 'categories') {
            count = pos_grid.products_sort.length;
            var search_pr = [];
            APP.db.values('products', Object.keys(pos_grid.tile_styles)).done(function(products){
                $.each(products, function (i, record) {
                    if(wc_pos_params.image_size == 'thumbnail'){
                        if(record.thumbnail_src){
                            record.featured_src = record.thumbnail_src;
                        }else{
                            if(record.images.length){
                                record.featured_src = record.images[0].src;
                            }
                        }
                    }

                    if (typeof record.type != 'undefined' && record.type == 'variable' && record.variations.length > 0 && pos_grid.tile_variables == 'tiles') {
                        var $li = $('<li id="product_' + record.id + '" class="title_product open_variantion category_cycle" data-id="' + record.id + '" title="'+record.name+'"><span></span><span class="price"></span></li>');
                    } else {
                        var $li = $('<li id="product_' + record.id + '" class="title_product add_grid_tile category_cycle" data-id="' + record.id + '" title="'+record.name+'"><span></span><span class="price"></span></li>');
                    }
                    $li.find('span').first().html(record.name);
                    var price = pos_get_price_html(record);
                    $li.find('span').last().html(price);

                    if (typeof pos_grid.tile_styles[record.id] != 'undefined' && pos_grid.tile_styles[record.id].style == 'colour' && !pos_grid.hide_text) {
                        $li.addClass("colour_tile");
                        $li.append('<span class="colour_label" style="background-color: #' + pos_grid.tile_styles[record.id].background + '"></span>');
                    } else {
                        if (typeof record.featured_src != 'undefined' && record.featured_src) {
                            $li.css({
                                'background-image': 'url(' + record.featured_src + ')'
                            });
                        } else {
                            $li.css({
                    ...
                    if (typeof record != 'undefined') {
                        if (online_only.indexOf(record.id) != -1) {
                            return true;
                        }
                    }
                    ul.append($li);
            ...
            count = pos_grid.products_sort.length;
            var search_pr = pos_grid.products_sort.slice(offset, count);
            APP.db.values('products', search_pr).done(function(products){
                $.each(products, function(index, product){
                    if(in_array(product.id, search_pr)){
                        if(wc_pos_params.image_size == 'thumbnail'){
                        ...
                        if (product.type == 'variable' && product.variations.length > 0 && pos_grid.tile_variables == 'tiles') {
                            var $li = $('<li id="product_' + product.id + '" class="title_product open_variantion category_cycle" data-id="' + product.id + '" title="'+product.name+'"><span></span><span class="price"></span></li>');
                        } else {
                            var $li = $('<li id="product_' + product.id + '" class="title_product add_grid_tile category_cycle" data-id="' + product.id + '" title="'+product.name+'"><span></span><span class="price"></span></li>');
                        }
                        $li.find('span').first().html(product.name);
                        var price = pos_get_price_html(product);
                        $li.find('span').last().html(price);

                        if (product.featured_src) {
                            $li.data('title', product.name).css({
                        ...
                        if (online_only.indexOf(product.id) != -1) {
                            return true;
                        }
                        ul.append($li);
                    }
            ...
            if (parent > 0 && $('#tile_go_back').length < 1) {
            ...
            if (search_pr.length > 0) {
                APP.db.values('products', search_pr).done(function (records) {
                    $.each(records, function (index, val) {
                        if (typeof val == 'undefined') return;

                        if(wc_pos_params.image_size == 'thumbnail'){
                            if(val.thumbnail_src){
                                val.featured_src = val.thumbnail_src;
                            }else{
                                if(val.images.length > 0){
                                    val.featured_src = val.images[0].src;
                                }
                            }
                        }
                        if (typeof val != 'undefined') {
                            ...
                            if (val.type == 'variable' && val.variations.length > 0 && pos_grid.tile_variables == 'tiles') {
                                var $li = $('<li id="product_' + val.id + '" class="title_product open_variantion category_cycle" data-id="' + val.id + '" title="'+val.name+'"><span></span><span class="price"></span></li>');
                            } else {
                                var $li = $('<li id="product_' + val.id + '" class="title_product add_grid_tile category_cycle" data-id="' + val.id + '" title="'+val.name+'"><span></span><span class="price"></span></li>');
                            }
                            $li.find('span').first().html(val.name);
                            ...
                            var current_el = $('#product_' + val.id);
    ...
    function filter_variantion(elems, parent, offset, limit, pagination) {
        if (typeof POS_TRANSIENT.grid_product_variations == 'undefined' || typeof POS_TRANSIENT.grid_product_attributes == 'undefined') {
            ...
            APP.db.get('products', parent).done(function(record){
                ...
                $.each(record.attributes, function (index, attr) {
                    if (attr.variation === true) {
                        var slug = !empty(attr.slug) ? attr.slug : str_replace(' ', '_', attr.name).toLowerCase();
                ...
                if (pagination === true) {
                    settings.breadcrumbs.append('<span class="sep"> → </span><span data-id="' + parent + '" class="prod_title">' + record.name + '</span>');
                }
    ...
    function build_filter_variantion(elems, parent, offset, limit, pagination) {
        ...
        var ul = $('#grid_layout_cycle ul');
        elems.hide();
        $('.add_grid_tile, .open_variantion, .open_variantion_attr, #tile_go_back, .product-loader').remove();
                    ...
                    $.each(options, function (index, val) {
                        if (in_array(val.slug, _options) || all_opt) {
                            ...
                            if (POS_TRANSIENT.grid_product_variations[variable_index]) {
                                var src = wc_pos_params.image_size == "thumbnail" ? "thumbnail_src" : "featured_src";
                                $li.data('title', title).css({
                                    'background-image': 'url(' + POS_TRANSIENT.grid_product_variations[variable_index].data[src] + ')'
                                });
                            }
                            ul.append($li);
                        }
