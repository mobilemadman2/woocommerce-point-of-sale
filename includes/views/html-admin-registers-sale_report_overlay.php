...
$ids = array();
$current_cashier = get_current_user_id();
$session = $wpdb->get_row("SELECT * from {$wpdb->prefix}wc_point_of_sale_sale_reports WHERE cashier_id = {$current_cashier} AND register_id = {$rg_id} ORDER BY id DESC LIMIT 1");
$report_opened = $session ? $session->opened : $data['opened'];
$report_closed = $session ? $session->closed : $data['closed'];
