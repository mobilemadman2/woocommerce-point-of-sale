...
<script type="text/template" id="tmpl-cart-product-item">
    ...
    <tr class="item_note new_row" id="item_note-{{cart_item_key}}">
        <td colspan="2">
            <input type="text" name="item_note" placeholder="<?php _e('Add a note...', 'wc_point_of_sale') ?>"
                   class="item_note" data-item="{{cart_item_key}}">
        </td>
        <td class="line_cost">
            <div class="view">
                {{#if editable}}
...
<script type="text/template" id="tmpl-cart-default-customer-item">
    ...
    if ($user_to_add > 0) {
        $customer = new WC_Customer($user_to_add);
        $user_data = $customer->get_data();
        $b_addr = $customer->get_billing_address();
        $s_addr = $customer->get_shipping_address();
        ?>
...
<script type="text/template" id="tmpl-cart-pos-discount">
    <tr data-coupon="POS Discount" class="tr_order_coupon order_coupon_pos_discount">
        <th class="coupon_label"><span class="discount-title">{{text}}</span><span
                    class="span_clear_order_coupon"><?php _e('Remove', 'wc_point_of_sale'); ?></span></th>
        <td class="coupon_amount">
            <span>
                <span class="formatted_coupon">{{{amount}}}</span>
...
<script type="text/template" id="tmpl-tabs-head">
    <span class="tab" data-tab_id="{{id}}">{{title}}</span>
</script>
<script type="text/template" id="tmpl-tabs-tab">
    <div class="tbc tab hide"
         id="tab-{{id}}" data-tab_id="{{id}}" data-spend_limit="{{limit}}" data-order_id="{{order_id}}"
