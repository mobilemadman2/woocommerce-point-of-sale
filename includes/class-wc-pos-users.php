...
class WC_Pos_Users {
  ...
  protected static $_instance = null;
    ...
    if( !defined('INIT_POS_USERS')){
      ...
      add_action( 'user_register', array($this, 'a'), 10, 1 );
        ...
        if ( ! empty( $user_query->results ) ) {
          foreach ( $user_query->results as $user ) {
            ...
            if (isset($_POST['_outlets_filter']) && !empty($_POST['_outlets_filter'])) {
              ...
              $user_outlet = (array)get_user_meta($user->data->ID, 'outlet', true);
              if(!in_array($outlet_id, $user_outlet)){
                  continue;
              }
            }
           ...
           $sales        = $this->get_user_sales($user->data->ID, $count_orders);
            $data[$user->data->ID] = array(
              'ID'         => $user->data->ID,
              'name'       => $user->data->display_name,
              'username'   => $user->data->user_nicename,
              'outlet'     => (array)get_user_meta( $user->data->ID, 'outlet', true ),
  ...
  public function get_user_sales($user_id, $count = 0)
  {
  	global $wpdb;
    if ( !$count ) {
      $count = 0;
    }
  	$query = "SELECT SUM(order_totals.total) from (
                SELECT meta.meta_value total FROM {$wpdb->posts} posts
  				...
  				LEFT JOIN {$wpdb->postmeta} served ON(served.post_id = posts.ID AND served.meta_key = 'wc_pos_served_by')
				WHERE posts.post_type = 'shop_order' 
				AND posts.post_status != 'wc-refunded'
				AND meta.meta_key = '_order_total' AND type.meta_value = 'POS' AND served.meta_value = {$user_id}
