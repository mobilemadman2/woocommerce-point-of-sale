        ...
        private function get_cash_balance()
        {
            ...
            if (isset ($this->register->detail->opening_cash_amount) && $this->register->detail->opening_cash_amount && $this->register->detail->opening_cash_amount->status) {
                ...
                $this->cash_data[] = array(
                    ...
                    'time' => (new WC_DateTime($this->register->detail->opening_cash_amount->time))->date_i18n(__('g:i:s A', 'woocommerce'))
                );
            }
            ...
            foreach ($results as $result) {
                ...
                if ($order->get_payment_method()  == 'cod') {
                    $paid = $order->get_meta('wc_pos_amount_pay');
                    $total = !empty($paid) ? floatval($paid) : $order->get_total();
                    ...
                    $this->cash_data[] = array(
                        ...
                        'time' => $order->get_date_created()->date_i18n(__('g:i:s A', 'woocommerce'))
                    );
                };
            }
